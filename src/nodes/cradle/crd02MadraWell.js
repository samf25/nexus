...
const MANUAL_PATTERN_SKILL_THRESHOLDS = Object.freeze([0, 2, 10, 25, 55, 100, 170, 270, 410, 600, 850, 1175]);
...
function normalizeCrd02Tab(tab) {
  const candidate = normalizeText(tab);
  return ["well", "soul", "combat", "soulfire", "grimoire"].includes(candidate) ? candidate : "well";
}

function normalizeRuntime(runtime) {
  const source = runtime && typeof runtime === "object" ? runtime : {};
  const activeTab = normalizeCrd02Tab(source.activeTab);
  ...
  return {
    ...
    manualPatternSkills: normalizeManualPatternSkills(source.manualPatternSkills),
...
function manualPatternSkillXpForLevel(level) {
  const targetLevel = Math.max(0, Math.floor(Number(level) || 0));
  if (targetLevel < MANUAL_PATTERN_SKILL_THRESHOLDS.length) {
    return MANUAL_PATTERN_SKILL_THRESHOLDS[targetLevel];
  }

  let xp = MANUAL_PATTERN_SKILL_THRESHOLDS[MANUAL_PATTERN_SKILL_THRESHOLDS.length - 1];
  for (let nextLevel = MANUAL_PATTERN_SKILL_THRESHOLDS.length; nextLevel <= targetLevel; nextLevel += 1) {
    xp += Math.round(325 * Math.pow(1.32, nextLevel - MANUAL_PATTERN_SKILL_THRESHOLDS.length));
  }
  return xp;
}

function manualPatternSkillLevelFromXp(xp) {
  const totalXp = Math.max(0, Math.floor(Number(xp) || 0));
  let level = 0;
  while (level < 80 && totalXp >= manualPatternSkillXpForLevel(level + 1)) {
    level += 1;
  }
  return level;
}
...
function manualPatternSkillLevel(runtime, patternIndex) {
  return manualPatternSkillLevelFromXp(manualPatternSkillEntry(runtime, patternIndex).xp);
}

function manualPatternSkillBonus(level) {
  const safeLevel = Math.max(0, Math.floor(Number(level) || 0));
  return Number((1 + ((safeLevel * safeLevel + 7 * safeLevel) / 200)).toFixed(3));
}

function manualPatternSkillThemeClass(level) {
  const safeLevel = Math.max(0, Math.floor(Number(level) || 0));
  if (safeLevel >= 7) return "lord";
  if (safeLevel >= 4) return "gold";
  if (safeLevel >= 2) return "core";
  return "early";
}

function manualPatternSkillStyle(level) {
  const safeLevel = Math.max(0, Math.floor(Number(level) || 0));
  if (safeLevel >= 7) {
    return "border-color:rgba(192,132,252,0.65);box-shadow:0 0 0 1px rgba(192,132,252,0.18),0 16px 42px rgba(88,28,135,0.18);";
  }
  if (safeLevel >= 4) {
    return "border-color:rgba(251,191,36,0.65);box-shadow:0 0 0 1px rgba(251,191,36,0.18),0 16px 42px rgba(146,64,14,0.16);";
  }
  if (safeLevel >= 2) {
    return "border-color:rgba(96,165,250,0.58);box-shadow:0 0 0 1px rgba(96,165,250,0.16),0 16px 42px rgba(30,64,175,0.14);";
  }
  if (safeLevel >= 1) {
    return "border-color:rgba(52,211,153,0.55);box-shadow:0 0 0 1px rgba(52,211,153,0.14),0 16px 42px rgba(6,78,59,0.12);";
  }
  return "border-color:rgba(148,163,184,0.32);";
}

function manualPatternSkillFillStyle(level, progressPct) {
  const safeLevel = Math.max(0, Math.floor(Number(level) || 0));
  const width = Math.min(100, Math.max(0, Number(progressPct) || 0));
  const color = safeLevel >= 7
  ...
  return `display:block;height:100%;width:${width.toFixed(2)}%;border-radius:999px;background:${color};`;
}

function manualPatternSkillView(runtime, patternIndex) {
  const index = normalizePatternIndex(patternIndex, MANUAL_PATTERNS.length - 1);
  const entry = manualPatternSkillEntry(runtime, index);
  const level = manualPatternSkillLevelFromXp(entry.xp);
  const currentThreshold = manualPatternSkillXpForLevel(level);
  const nextThreshold = manualPatternSkillXpForLevel(level + 1);
  const xpIntoLevel = Math.max(0, entry.xp - currentThreshold);
  ...
  return {
    ...
    xpIntoLevel,
    xpForNext,
    progressPct: Math.min(100, Math.max(0, (xpIntoLevel / xpForNext) * 100)),
    theme: manualPatternSkillThemeClass(level),
  };
}
...
function manualMadraGain(runtime, patternIndex = null) {
  ...
  const patternSkillMultiplier = patternIndex == null ? 1 : manualPatternSkillBonus(manualPatternSkillLevel(runtime, patternIndex));
  return Math.max(1, Math.round(base * stageMultiplier * prestigeMultiplier * patternSkillMultiplier));
}
...
export function initialCrd02Runtime(context = {}) {
  ...
  return {
    ...
    manualPatternSkills: defaultManualPatternSkills(),
    lastTickAt: now,
    cycling: {
      twinStarsLevel: 0,
      heavenEarthLevel: 0,
...
export function reduceCrd02Runtime(runtime, action) {
  ...
  if (action.type === "crd02-open-tab") {
    return {
      ...current,
      activeTab: normalizeCrd02Tab(action.tab),
    };
  }
  ...
  if (action.type === "crd02-manual-tap") {
    ...
    if (streak >= MANUAL_STREAK_TARGET) {
      const patternIndex = current.manual.patternIndex;
      const beforeSkill = manualPatternSkillView(current, patternIndex);
      const gain = manualMadraGain(current, patternIndex);
      ...
      const next = {
        ...
        manualPatternSkills: nextManualPatternSkills,
        manual: {
          ...current.manual,
          open: false,
          streak: 0,
        ...
        lastMessage: `Manual cycle complete. +${gain} madra.${levelMessage}`,
      };
      return {
        ...next,
        solved: solveState(next),
...
function manualModalMarkup(runtime, manualReward) {
  ...
  const patternIndex = runtime.manual.patternIndex;
  const pattern = manualPatternByIndex(patternIndex);
  const skill = manualPatternSkillView(runtime, patternIndex);
  ...
  return `
    <div class="crd02-manual-modal" role="dialog" aria-label="Manual Cultivation">
      <section class="crd02-manual-surface">
        <header>
          <h3>Manual Cultivation <span class="crd02-cycle-chip is-strong">+${escapeHtml(String(manualReward))} Madra</span></h3>
...
function cyclingGrimoireMarkup(runtime) {
        ...
        <div class="crd02-cycle-copy">
          ...
          <div class="crd02-grimoire-xp">
            <div class="crd02-grimoire-xp-track" style="height:8px;width:100%;overflow:hidden;border-radius:999px;background:rgba(148,163,184,0.16);" aria-hidden="true">
              <span class="crd02-grimoire-xp-fill" style="${escapeHtml(manualPatternSkillFillStyle(skill.level, skill.progressPct))}"></span>
            </div>
            <small>${escapeHtml(String(skill.xpIntoLevel))}/${escapeHtml(String(skill.xpForNext))} XP to Level ${escapeHtml(String(nextLevel))}</small>
          </div>
        </div>
      </article>
    `;
  ...
  return `
    <section class="crd02-panel crd02-grimoire-panel">
      <div class="crd02-header-title-row">
...
export function renderCrd02Experience(context) {
  ...
  const activeTab = normalizeCrd02Tab(runtime.activeTab);
  ...
  const wellPanel = canSeeMenus
    ...
    `;
  const grimoirePanel = cyclingGrimoireMarkup(runtime);
      ...
      <section class="crd02-header">
        <div class="crd02-header-copy">
          ...
          <div class="toolbar crd02-header-tabs">
            ...
            <button type="button" data-node-id="${NODE_ID}" data-node-action="crd02-open-tab" data-tab="grimoire" ${activeTab === "grimoire" ? "disabled" : ""}>Cycling Grimoire</button>
            ${
